...
export function WorkGrid() {
  return (
    <section id="work" className="min-h-[100dvh] md:min-h-screen py-16 md:py-32 bg-deep-basalt relative flex flex-col justify-center overflow-hidden">
      <div className="container mx-auto px-4 flex flex-col h-full md:h-auto justify-center">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-6 md:mb-16 gap-4 md:gap-6">
          <div className="max-w-2xl">
            <SectionBadge label="THE WORK" className="mb-3 md:mb-6" />
            <h2 className="text-3xl md:text-5xl font-display font-bold mb-2 md:mb-6 text-white">
              <span className="text-gradient-blue">Work</span> That Ships.
            </h2>
            <p className="text-gray-300 text-base md:text-lg font-light leading-relaxed hidden md:block">
        ...
        <MobileCarousel
          ...
          renderItem={(project) => (
            <Link key={project.id} href={project.link} className="snap-center shrink-0 w-[90vw] max-w-[340px]">
              <div className="h-full glass-card rounded-2xl overflow-hidden relative min-h-[280px] flex flex-col">
                ...
                <div className="relative p-5 flex flex-col h-full z-10">
                  ...
                  <div className="flex-1 flex flex-col justify-between">
                    <div className="space-y-2">
                      ...
                      <p className="text-gray-400 font-light text-sm leading-snug">{project.subtitle}</p>
                    </div>
                    <div className="mt-4 -mb-2 mx-0">
                      <AbstractBrowser variant={project.browserVariant} className="w-full shadow-xl opacity-90 rounded-lg overflow-hidden" />
                    </div>
                  </div>
                </div>
              </div>
                ...
                <div className="relative p-8 flex flex-col h-full z-10">
                  ...
                  <div className={`flex flex-col flex-1 ${project.browserPosition === 'right' ? 'md:flex-row md:items-end gap-8' : 'justify-between gap-8'}`}>
                    <div className={`space-y-3 ${project.browserPosition === 'right' ? 'md:max-w-[40%] pb-4' : ''}`}>
                      ...
                      <p className="text-gray-400 font-light text-lg leading-snug">{project.subtitle}</p>
                    </div>
                    <div className={`relative ${
                      ...
                      <AbstractBrowser variant={project.browserVariant} className="w-full shadow-2xl shadow-black/50 origin-top" />
                    </div>
                  </div>
                </div>
              </motion.div>
