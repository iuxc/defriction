...
export function FooterContact({ title = "Ready to start?", className, stickyClassName, stickyVisible = true, backLink, alwaysSticky = false, withGradientShadow = false, monashSwitcher = false, disableExpansion = false, glowColor = "blue", removeTextShadow = false, infoContent }: FooterContactProps) {
  ...
  return (
    <div ref={containerRef} className={cn("w-full px-4 relative z-10 font-sans", className)}>
          ...
          <>
            ...
            {showSticky && withGradientShadow && (
              <motion.div
                ...
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                className="fixed bottom-0 left-0 right-0 h-48 bg-gradient-to-t from-deep-basalt via-deep-basalt/80 to-transparent pointer-events-none z-[75]"
              />
            )}
                      ...
                      <div className="relative z-10">
                        <div className="flex items-center justify-between mb-4">
                          <div className="flex items-center gap-3">
                            ...
                            <h3 className="text-lg font-bold text-white font-display">What You Need To Know</h3>
                          </div>
                          <button
                        ...
                        <div className="text-gray-300 leading-relaxed text-sm space-y-3 font-light">
                          {infoContent || (
                            <>
                              <p>Monash applicants currently navigate dozens of pages to figure out if they qualify. This prototype replaces that maze with 3–7 questions and a clear answer.</p>
                              <p>It also reveals equity bonuses most students don't know they have—helping first-gen and rural applicants see they're closer than they think.</p>
                            </>
                          )}
                        </div>
                      </div>
            ...
            <motion.div
              layoutId="contact-card"
              className={cn(
                "rounded-[2rem] border border-white/10 hover:border-white/20 cursor-pointer glass-dark",
                !monashSwitcher && "overflow-hidden",
                showSticky
                  ? cn("fixed bottom-6 left-4 right-4 md:left-1/2 md:-translate-x-1/2 md:w-[90%] md:max-w-4xl z-[80] border-white/20 block", showSticky && GLOW_CLASS_MAP[glowColor], stickyClassName)
                  : cn("mx-auto relative border-white/20", disableExpansion ? "max-w-4xl" : "max-w-3xl")
              )}
              onClick={() => {
                if (monashSwitcher) return;
                setIsOpen(true);
              }}
